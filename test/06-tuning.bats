...
@test "tuning adds a C note when none is present in the score" {
  tuning=$(xslt3 -xsl:src/xsl/tuning.xsl -s:test/data/tuning-no-c.musicxml)
  run echo $tuning
  assert_output --partial '@ABL NOTE_NAMES "C"'
}
